...
import { newProject, deleteProject, getProject, updateProject } from './project.service.js';
...
export const httpNewProject = catchAsync(async (req, res) => {
  ...
  const project = await newProject(user.id, title, description, currentTeam, deadline);

  if (!project) {
    throw new AppError('Error in creating the project', 500);
  }

  currentTeam.projects.push(project.id);
...
export const httpGetTeamProjects = catchAsync(async (req, res) => {
  ...
  return AppResponse(res, 200, members , 'Team Projects fetched successfully');
});
