...
export const httpGetCurrentUser = catchAsync(async (req, res) => {
  const { user } = req;
  return AppResponse(res, 200, EntityTransformer(user), 'Current user fetched successfully');
});
...
export const httpUpdateUser = catchAsync(async (req, res) => {
  ...
  return AppResponse(res, 200, EntityTransformer(updatedUser), 'User updated successfully');
});
